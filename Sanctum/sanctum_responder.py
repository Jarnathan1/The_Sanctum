...
prompt_path = base_path / "Prompt_Root"
response_path = base_path / "Prompt_Responses"
memory_path = base_path / "Memory_Scrolls"

# Load memory fragments (1 to 3 depending on internal decision)
def gather_fragments():
    files = list(memory_path.glob("*.txt"))
    if not files:
        return []
    use_multiple = random.random() < 0.5  # 50% chance to go deeper
    count = 1 if not use_multiple else random.randint(2, 3)
